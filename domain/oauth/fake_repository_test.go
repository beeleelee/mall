...
func newFakeClientRepo() *fakeClientRepo {
	return &fakeClientRepo{
		byID:  make(map[kernel.ID]*OAuthClient),
		byCID: make(map[string]*OAuthClient),
	}
}

func (f *fakeClientRepo) Save(_ context.Context, c *OAuthClient) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.byID[c.ID] = c
...
func (f *fakeClientRepo) FindByClientID(_ context.Context, clientID string) (*OAuthClient, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeClientRepo) FindByID(_ context.Context, id kernel.ID) (*OAuthClient, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	c, ok := f.byID[id]
	if !ok {
...
func (f *fakeCodeRepo) Save(_ context.Context, c *AuthorizationCode) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.codes[c.Code] = c
	return nil
...
func (f *fakeCodeRepo) FindByCode(_ context.Context, code string) (*AuthorizationCode, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	c, ok := f.codes[code]
	if !ok {
...
func (f *fakeTokenRepo) Save(_ context.Context, t *RefreshToken) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.tokens[t.ID] = t
	return nil
...
func (f *fakeTokenRepo) FindByID(_ context.Context, id string) (*RefreshToken, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	t, ok := f.tokens[id]
	if !ok {
